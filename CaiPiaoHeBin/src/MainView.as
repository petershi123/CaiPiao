package
{
	...
	public class MainView extends MovieClip
	{
		...
		private var _parentHe:CaiPiaoHeBin;
		private var regetQihao:Boolean;
		
		public function MainView(_par:CaiPiaoHeBin)
		{
			super();
			_parentHe = _par;
		}
		public function init():void
		{
			...
			txt5=_main.getChildByName("txt5") as TextField;
			txt6=_main.getChildByName("txt6") as TextField;
		...
		private function selectType():void
		{
			...
			urlloader.load(new URLRequest(CaiPiaoHeBin._preURL+"Soft/link.xml"));	//CaiPiao._preURL+"/blt168/link.xml"));
			urlloader.addEventListener(Event.COMPLETE,			complete);
			urlloader.addEventListener(IOErrorEvent.IO_ERROR,	onIOError);
			function complete(evt:Event):void
			{
				var _xml:XML=new XML(evt.currentTarget.data as String);
				//_xml.link.@src = "http://www.fl776.com/kai/linkb3239765.asp";
				...
				{
					_dateLink=CaiPiaoHeBin._preURL+"getqi.aspx?type=1";
					isOutLinkBoo = false;
				}
		...
		private function getlinkHao():void
		{
			...
			function complete(evt:Event):void
			{
				...
				/*if(String(arr[0]).substr(String(arr[0]).length-2,String(arr[0]).length) != "50")
				{
					_qihao=int(arr[0])+1;
				}
				else
				{
					_qihao = int(int(int(String(arr[0]).substr(0,String(arr[0]).length-2))+1)+"01");
				}*/
				trace("get qihao is :" + _qihao);
				resultArr = [];
				...
				txt3.htmlText="<font color=\"#FFFF00\">"+_qihao+"</font>";
			}
			function onIOError(err:IOErrorEvent):void
			{
				trace(err.toString());
		...
		private function onShowHandler(evt:Event):void
		{
			...
			nowdate = new Date();
			nowdate.setTime(nowdate.getTime() + CaiPiaoHeBin.timeOffset);
			...
			if(_parentHe.kuaishanOpenTime == -1)
			{
				txt5.text = "";
				txt6.text = "";
			}
			...
			{
				showKuaiShanTime(nowdate);
			}
		}
		
		//显示快三时间
		private function showKuaiShanTime(curDate:Date):void
		{
			...
			var leftTime:Number;
			if(_parentHe.kuaishanOpenTime <= curDate.time)
			{
				leftTime = 10*60*1000 + _parentHe.kuaishanOpenTime - curDate.time;
			}
			else
			{
				leftTime = _parentHe.kuaishanOpenTime - curDate.time;
			}
			...
			txt6.text = "快三:"+(_loc_11 < 10 ? ("0" + _loc_11) : (_loc_11)) + ":" + (_loc_12 < 10 ? ("0" + _loc_12) : (_loc_12));
			txt5.text = _parentHe.kuaiqihao;
		}
		private var chageFaceBoo:Boolean = false;	// 视频切换
		...
		private function showTimeAndOther(year:Number,month:Number,date:Number,day:Number,hour:Number,minute:Number,second:Number):void
		{
			...
			{
				time=int((num-10)/2+1)+1;
				leftTime=(14-(num-10)%15)*60+60-second;
			}
			
			if(leftTime % 30 == 0)//sfloader.visible = false;ksloader.visible = true;
			{
				stage.dispatchEvent(new Event("ChangeFace",true));
			}
			else if(leftTime <= 2)
			{
				if(!chageFaceBoo)
				{
					stage.dispatchEvent(new Event("ShowTen"));
					chageFaceBoo = true;
				}
			}
			...
			if((hour<9) || (hour>21) || ((hour==21) && (minute>30)))	//停服了
			{
				...
				{
					endMc.visible=true;
					stage.dispatchEvent(new Event("hideKuaiLeiTen"));
				}
				if(loader!=null)
				{
					loader.unload();
			...
			{
				txt3.htmlText="<font color=\"#FFFF00\">"+(_qihao+1)+"</font>";
			}
			...
			if(_isSelectPrizeBo==false)
			{
				...
				txt4.htmlText="开奖:<font color=\"#FFFF00\">"+lefttimeStr+":"+leftsecondStr+"</font>";
			...
			if(leftTime<2)
			{
				if(timestopBo==false)
				{
					...
					txt4.htmlText="开奖:<font color=\"#FFFF00\">"+"00:00"+"</font>";
		...
		{
			var urlloader:URLLoader = new URLLoader();
			urlloader.load(new URLRequest(CaiPiaoHeBin._preURL+"OffLine.aspx?user="+CaiPiaoHeBin.userName));
		...
		private function reset(evt:Event):void
		{
			...
			{
				var urlloader:URLLoader=new URLLoader();
				urlloader.load(new URLRequest(CaiPiaoHeBin._preURL+"Soft/linktest.xml"));	
				urlloader.addEventListener(Event.COMPLETE,			complete);
				urlloader.addEventListener(IOErrorEvent.IO_ERROR,	onIOError);
			}
			else
		...
		private function unloadComplete():void
		{
			...
			trace("dispatchEvent showThree....");
			stage.dispatchEvent(new Event("ShowThree"));
		...
		private function openPrizeHandler(evt:Event):void
		{
			...
			resultLoader.content["localDataLink"] = CaiPiaoHeBin._preURL;
